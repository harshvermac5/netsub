...
def generate_eui64_address():
    global_routing_prefix = ':'.join([prefix_entries[i].get() for i in range(8)])
...
# Create entry widgets for each quartet of the global routing prefix
prefix_entries = [Entry(window, width=4) for _ in range(8)]
...
# Create entry widgets for each pair of the MAC address
mac_entries = [Entry(window, width=2) for _ in range(6)]
...
generate_button.grid(row=2, column=0, columnspan=9, pady=10)

result_text = Text(window, height=2, width=40, wrap=tk.NONE, state=tk.DISABLED)
result_text.grid(row=3, column=0, columnspan=9, pady=10)

result_scrollbar = Scrollbar(window, command=result_text.yview)
result_scrollbar.grid(row=3, column=9, sticky='nsew')
